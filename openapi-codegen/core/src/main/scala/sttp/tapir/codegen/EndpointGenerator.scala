  ...
  def endpointDefs(
    ...
    val endpointDecls = geMap.map { case (k, ge) =>
      val definitions = ge
        ...
        .mkString("\n")
      val allEP = s"lazy val $allEndpoints: List[sttp.tapir.AnyEndpoint] = List(${ge.map(_._1).mkString(", ")})"
      ...
      .map { param =>
        param.schema match {
          case st: OpenapiSchemaSimpleType =>
            ...
            s""".in(${param.in}[$t]("${param.name}")$desc)""" -> None
          ...
          case e @ OpenapiSchemaEnum(_, _, _) =>
            ...
            val explode = param.explode.getOrElse(false)
            ...
            val tpe = if (explode) s"List[$enumName]" else enumName
            val desc = param.description.map(d => JavaEscape.escapeString(d)).fold("")(d => s""".description("$d")""")
