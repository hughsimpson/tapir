...
object OpenapiModels {
  ...
  case class OpenapiPath(
      ...
      specificationExtensions: Map[String, ReifiableRenderableValue] = Map.empty
  )
  ...
  implicit val PartialOpenapiPathMethodDecoder: Decoder[OpenapiPathMethod] = { (c: HCursor) =>
    for {
      ...
      specificationExtensions = specificationExtensionKeys
        .flatMap(key => c.downField(key).as[ReifiableRenderableValue].toOption.map(key.stripPrefix("x-") -> _))
        .toMap
    } yield {
      OpenapiPathMethod(
        "--partial--",
