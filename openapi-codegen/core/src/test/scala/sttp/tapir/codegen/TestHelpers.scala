    ...
    Seq(
      OpenapiPath(
        ...
        specificationExtensions = Map(
          "custom-string-extension-on-path" -> ReifiableValueString("foobar"),
          "custom-list-extension-on-path" -> ReifiableValueList(
            Vector(ReifiableValueString("foo"), ReifiableValueString("bar"))
          ),
          "custom-map-extension-on-path" -> ReifiableValueMap(
            Map(
              "bazkey" -> ReifiableValueString("bazval"),
              "quuxkey" -> ReifiableValueList(
                Vector(ReifiableValueString("quux1"), ReifiableValueString("quux2"))
              )
            )
          )
        )
      ...
      OpenapiPath(
        ...
        methods = Seq(
          OpenapiPathMethod(
            ...
            requestBody = None,
            specificationExtensions = Map(
              "custom-string-extension-on-operation" -> ReifiableValueString("bazquux"),
              "custom-list-extension-on-operation" -> ReifiableValueList(
                Vector(ReifiableValueString("baz"), ReifiableValueString("quux"))
              ),
              "custom-map-extension-on-operation" -> ReifiableValueMap(
                Map(
                  "bazkey" -> ReifiableValueString("bazval"),
                  "quuxkey" -> ReifiableValueList(
                    Vector(ReifiableValueString("quux1"), ReifiableValueString("quux2"))
